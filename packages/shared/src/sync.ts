 ...
 * 走同步器的那两个来源。
 *
 * `local.ics` 也是一个外部来源，但它是用户拖进来的一份文件，没有会话、没有轮询，
 * 所以同步那条路上的函数只收这两个。
 */
export const RucExternalSource = z.enum(['ruc.portal', 'ruc.graduate'])
export type RucExternalSource = z.infer<typeof RucExternalSource>
