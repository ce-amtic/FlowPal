...
- 界面上所有面向用户的文字（含模型生成的）：正式、清晰、简明；不口语化、不造黑话、不写面向开发过程的元语句 ([[011-ui-copy-style]])
- 一条判断进不进待确认，看错了的代价是否不可逆，不看模型的信心；默认让系统自己走完，用户零介入 ([[015-needs-confirm-threshold]])
- Agent 与通用助手的边界是发起权，不是能力大小：主路径上产品先说话、模型每句话有可指出的凭据、真相在库里不在对话里 ([[018-agent-boundary-is-initiative]])
...
- **五类条目** = 事件（有确定时间点的外部节点）、事务（用户要做的事）、想法（专注中扔进来的零散念头）、进度（某个项目做到哪）、状态（用户自评或系统观察）。
- **项目** = 条目之上的一层长期追踪对象：一门课、一次申请、一篇论文。条目可归属某个项目，进度挂在项目上。
- **循环 / agent 循环** = 写入库的唯一机制：一次带工具的模型调用序列，输入是一条碎片，输出是若干工具调用加一句给用户看的话。抽取与合并都在其中完成。
